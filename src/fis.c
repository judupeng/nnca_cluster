...
/* find number x in "******=x" */
static DOUBLE getNumber(char *buf, FILE *fp)
...
/* find string x in "*******='x'" */
static void getString(char *buf, FILE *fp, DOUBLE *array)
{
	int i;
...
static int getArray(char *string, DOUBLE *array)
{
	int i;
	int start, end, index;
	char tmp[STR_LEN];
...
static void getMfN(char *filename, int in_n, DOUBLE *in_mf_n, int out_n, DOUBLE *out_mf_n)
{
	int i, tmp;
	char buf[STR_LEN];
...
static DOUBLE ** returnEmptyFismatrix(char *filename, int *row_n_p, int *col_n_p)
...
static DOUBLE ** returnFismatrix(char *fis_file, int *row_n_p, int *col_n_p)
{
	int i, j, k;
	FILE *fp;
	char buf[STR_LEN];
